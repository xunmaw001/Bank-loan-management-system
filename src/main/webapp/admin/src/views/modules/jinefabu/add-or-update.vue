<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="利息" prop="lixi">
          <el-input v-model="ruleForm.lixi" 
              placeholder="利息" clearable  :readonly="ro.lixi"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="利息" prop="lixi">
              <el-input v-model="ruleForm.lixi" 
                placeholder="利息" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
...
export default {
  ...
  computed: {
    jine: {
      get: function () {
        return 0+parseFloat(this.ruleForm.daikuanjine==""?0:this.ruleForm.daikuanjine)+parseFloat(this.ruleForm.lixi==""?0:this.ruleForm.lixi) || 0
      }
    },
  },
  created() {
  ...
  methods: {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `jinefabu/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.jinefabuCrossAddOrUpdateFlag = false;
                  this.parent.search();
